...
import com.inso.plugin.dao.WatchUserDao;
...
import com.inso.plugin.sync.http.bean.HttpStepHistory;
import com.inso.plugin.sync.http.bean.HttpWorldCity;
...
import static com.inso.plugin.provider.DBColumn.PreferCityColumns.IS_SEL;
import static com.inso.plugin.provider.DBColumn.VIP.CONTACTID;
import static com.inso.plugin.tools.Constants.ConfigVersion.*;
import static com.inso.plugin.tools.Constants.*;
import static com.inso.plugin.tools.Constants.SystemConstant.*;
import static com.inso.plugin.tools.Constants.TimeStamp.*;
...
//    }
    ...
    public boolean syncWorldCity(List<HttpWorldCity> src) {
        ...
        try {
            ...
            for (final HttpWorldCity item : src) {
                ...
                values.put(IS_SEL, item.select ? 1 : 0);
    ...
    public synchronized List<PreferCitiesDao> getAllPreferCities() {
        ...
        try {
            ...
            if (cursor.moveToFirst()) {
                do {
                    Boolean b0 = cursor.getInt(cursor.getColumnIndexOrThrow(IS_SEL)) == 1;
    ...
    public String getSettingZone() {
        ...
        try {
            ...
            cursor = rDb.query(DBColumn.PreferCityColumns.TABLE_NAME, fetch_columns, IS_SEL + "=?", new String[]{"1"}, null, null, null, null);
            if (cursor.moveToFirst()) {
                do {
