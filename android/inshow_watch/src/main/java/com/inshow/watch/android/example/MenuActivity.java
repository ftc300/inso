...
import com.xiaomi.smarthome.device.api.IXmPluginHostActivity.BleMenuItem;
import com.xiaomi.smarthome.device.api.IXmPluginHostActivity.InfoMenuItem;
...
public class MenuActivity extends XmPluginBaseActivity implements View.OnClickListener {
    ...
    private void openMenu3() {
        ...
        hostActivity().openMoreMenu2(items, false, MENU2_CODE, null);
    ...
    private void openSubMenu1() {
        ...
        hostActivity().openMoreMenu(items, true, SUBMENU1_CODE, intent);
    }
                    ...
                    if (!TextUtils.isEmpty(name)) {
                        ...
                        if (name.equals("StringMenu1")) {
                            ...
                            MLAlertDialog.Builder builder =
                                    new MLAlertDialog.Builder(MenuActivity.this)
                                            .setTitle("设置名字")
                                            .setInputView("请输入名称", true)
                                            .setPositiveButton("确定",null)
                                            .setNegativeButton("取消", null);
                            final  MLAlertDialog dialog = builder.show();
                            dialog.setOnDismissListener(new DialogInterface.OnDismissListener() {
                                @Override
                                public void onDismiss(DialogInterface dialogInterface) {
                                    L.e("rename:" + dialog.getInputView().getText().toString());
                                    String input = dialog.getInputView().getText().toString();
                                    if (!TextUtils.isEmpty(input)) {
                                        XmBluetoothManager.getInstance().deviceRename(MAC, input);
                                        XmPluginHostApi.instance().modDeviceName(MAC, input, new Callback<Void>() {
                                            @Override
                                            public void onSuccess(Void aVoid) {
                                                L.e("rename:onSuccess");
                                            }

                                            @Override
                                            public void onFailure(int i, String s) {
                                                L.e("rename:onFailure");
                                            }
                                        });
                                    }
                                }
