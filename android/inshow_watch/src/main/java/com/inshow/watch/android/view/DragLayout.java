...
public class DragLayout extends ViewGroup {
	...
	private static final int VEL_THRESHOLD = 80; // 滑动速度的阈值，超过这个绝对值认为是上下
	...
	private ShowPrePageNotifier prePageListener; // 手指松开是否加载下一页的notifier
	private int mCurrentIndex = 1;
	private int horMinDistance =200;
	private int minVelocity  = 0;
	private Context mContext;
	private GestureDetector mGestureDetector ;

	public void setShowLeftOrRightPageListener(ShowLeftOrRightPageListener showLeftOrRightPageListener) {
		this.showLeftOrRightPageListener = showLeftOrRightPageListener;
	}

	private ShowLeftOrRightPageListener showLeftOrRightPageListener;
	...
	public DragLayout(Context context, AttributeSet attrs, int defStyle) {
		...
		mGestureDetector = new GestureDetector(mContext, new GestureDetector.OnGestureListener() {
			...
			public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX , float velocityY ) {
				try {
					//to left
					if (mCurrentIndex==1&&e1.getX() - e2.getX() > horMinDistance && Math.abs(velocityX) > minVelocity&&Math.abs(e1.getY()-e2.getY()) < Math.abs(e1.getX()-e2.getX())) {
						if (showLeftOrRightPageListener!=null)
							showLeftOrRightPageListener.onDragToLeft();
					} else if (mCurrentIndex==1&&(e2.getX() - e1.getX() > horMinDistance && Math.abs(velocityX) > minVelocity)&&Math.abs(e1.getY()-e2.getY()) < Math.abs(e1.getX()-e2.getX())) {//to right
						if(showLeftOrRightPageListener!=null)
							showLeftOrRightPageListener.onDragToRight();
					}
				}catch (Exception e) {
					e.printStackTrace();
				}
	...
	private void animTopOrBottom(View releasedChild, float yvel) {
		...
		if (releasedChild == frameView1) {
			...
			if (yvel > VEL_THRESHOLD && (downTop1 == -viewHeight && releasedChild.getTop() > DISTANCE_THRESHOLD)) {
				...
				// 上一页可以初始化了
				if (null != prePageListener) {
					prePageListener.onDragNext();
				}
			}
		}
	...
	public void setPrePageListener(ShowPrePageNotifier prePageListener) {
	...
	public  interface  ShowLeftOrRightPageListener
	{
